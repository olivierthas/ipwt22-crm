...
<div class="buttons">
{if $bean->aclAccess("save")}<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="button primary" onclick="var _form = document.getElementById('EditPassword'); {if $isDuplicate}_form.return_id.value=''; {/if}_form.action.value='Save'; if(check_form('EditPassword'))SUGAR.ajaxUI.submitForm(_form);return false;" type="submit" name="button" value="{$APP.LBL_SAVE_BUTTON_LABEL}" id="SAVE">{/if} 
...
{counter name="panelFieldCount"  print=false}
<input tabindex="0"  type="password" name="new_secret" id="new_secret" placeholder="{$MOD.LBL_LEAVE_BLANK}" size="30" autocomplete="dont-fill-this-password"><input tabindex="0"  type="hidden" name="allowed_grant_type" id="allowed_grant_type" value="password"><br /><span>{$MOD.LBL_REMEMBER_SECRET}</span>
</div>
...
YAHOO.util.Event.onContentReady("EditPassword",
    function () {ldelim} initEditView(document.forms.EditPassword) {rdelim});
